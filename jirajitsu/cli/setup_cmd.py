...
def setup(ctx):
    ...
    # Migration scoping (optional - can be configured later)
    if Confirm.ask("Configure migration scoping options now?", default=False):
        console.print("[dim]These settings determine which issues to migrate and how to handle custom fields[/dim]")
        jira_filter_id = Prompt.ask("JIRA Filter ID for migration (optional)", default=str(existing_config.get('jira_filter_id', '')))
        jira_tag_field = Prompt.ask("JIRA custom field for tags (optional)", default=existing_config.get('jira_tag_field', ''))
    else:
